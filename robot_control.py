...
class RobotControl:
    ...
    def flip_cube(self):
        ...
        if self.motor_flip:
            self.motor_flip.rotate_to_angle(self._get_motor_angle(robot_config.FLIPPER_FLIP_ANGLE, robot_config.GEAR_RATIO_FLIPPER), 200) 
            self.motor_flip.wait_for_movement()
            self.motor_flip.rotate_to_angle(self._get_motor_angle(robot_config.FLIPPER_RESET_ANGLE, robot_config.GEAR_RATIO_FLIPPER), 200) 
            self.motor_flip.wait_for_movement()
    ...
    def move_sensor_arm(self, target_arm_angle):
        ...
        if self.motor_arm:
            self.motor_arm.rotate_to_angle(motor_angle, 300)
    ...
    def hold_cube(self):
        """Sjede ramenem do poloviny, aby přidržel vrchní dvě vrstvy."""
        print("Robot: Hold Cube (Přidržení)")
        if self.motor_flip:
            target = self._get_motor_angle(robot_config.FLIPPER_HOLD_ANGLE, robot_config.GEAR_RATIO_FLIPPER)
            self.motor_flip.rotate_to_angle(target, 200)
            self.motor_flip.wait_for_movement()
            self.motor_flip.hold()

    def release_cube(self):
        """Zvedne rameno zpět nahoru do výchozí polohy."""
        print("Robot: Release Cube (Uvolnění)")
        if self.motor_flip:
            target = self._get_motor_angle(robot_config.FLIPPER_RESET_ANGLE, robot_config.GEAR_RATIO_FLIPPER)
            self.motor_flip.rotate_to_angle(target, 200)
            self.motor_flip.wait_for_movement()
            self.motor_flip.hold()